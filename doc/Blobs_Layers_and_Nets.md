...
As data and derivatives flow through the network in the forward and backward passes, Caffe stores, communicates, and manipulates the information as blobs: **the blob is the standard array and unified(标准的) memory interface for the framework.** The layer comes next as the foundation of both model and computation. The net follows as the collection and connection of layers. The details of blob describe how information is stored and communicated in and across layers and nets.
...
A Blob is a wrapper over the actual data being processed and passed along by Caffe, and also *under the hood(底层)* provides synchronization capability between the CPU and the GPU. Mathematically, a blob is an N-dimensional array stored in a C-contiguous fashion.

Caffe stores and communicates data using blobs. Blobs provide a unified memory interface holding data; **e.g., batches of images, model parameters, and derivatives for optimization.**

Blobs conceal(隐藏) the computational and *mental overhead(精神上的开销)* of mixed CPU/GPU operation by synchronizing from the CPU host to the GPU device as needed. Memory on the host and device is allocated on demand (lazily) for efficient memory usage.
...
- Number / N is the batch size of the data. Batch processing achieves better throughput(吞吐量) for communication and device processing. For an ImageNet training batch of 256 images N = 256.
...
// And the object we operate is `blob`
const Dtype* foo;
Dtype* bar;
...
The layer is the essence of a model and the fundamental unit of computation. Layers convolve filters, pool, take inner products, apply nonlinearities like rectified-linear and sigmoid and other elementwise transformations, normalize, load data, and compute losses like softmax and hinge. [See the layer catalogue](http://caffe.berkeleyvision.org/tutorial/layers.html) for all operations. Most of the types needed for state-of-the-art deep learning tasks are there.

![](http://okye062gb.bkt.clouddn.com/2017-08-14-030313.jpg)
...
![](http://okye062gb.bkt.clouddn.com/2017-08-14-031115.jpg)
